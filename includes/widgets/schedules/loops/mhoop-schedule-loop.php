<?php
	$curYear       = $mbball_term_year_name;
	$curYear_title = substr($curYear, 2);
	$prevYear      = $mbball_term_year_name - 1;
	$lnkYears      = $prevYear . '-' .$curYear;
	$title_years   = $prevYear . '-' .$curYear_title;
	$sched_link    = get_bloginfo('url') . '/' . $lnkYears . '-mens-basketball-schedule';
?>
<div class="schedule-block">
	<table>
		<?php
				echo "<tr class=\"row sched-year-title\"><th colspan=\"3\"><h4>$title_years Men's Hoops Schedule</h4></th></tr>";
				if($mhoops_loop->have_posts()){
					while($mhoops_loop->have_posts()){
						$mhoops_loop->the_post();
