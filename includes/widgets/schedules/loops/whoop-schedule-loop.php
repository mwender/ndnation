<?php
	$curYear       = $wbball_term_year_name;
	$curYear_title = substr($curYear, 2);
	$prevYear      = $wbball_term_year_name - 1;
	$lnkYears      = $prevYear . '-' .$curYear;
	$title_years   = $prevYear . '-' .$curYear_title;
	$sched_link    = get_bloginfo('url') . '/' . $lnkYears . '-womens-basketball-schedule';
?>
<div class="schedule-block">
	<table>
		<?php
				echo "<tr class=\"row sched-year-title\"><th colspan=\"3\"><h4>$title_years Women's Hoops Schedule</h4></th></tr>";
				if($whoops_loop->have_posts()){
					while($whoops_loop->have_posts()){
						$whoops_loop->the_post();
